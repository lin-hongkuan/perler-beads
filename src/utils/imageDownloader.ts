...
export async function downloadImage({
  ...
  const processDownload = () => {
    ...
    let downloadHeight = titleBarHeight + gridHeight + (axisLabelSize * 2) + statsHeight + extraTopMargin + extraBottomMargin + footerBrandHeight;
    ...
    ctx.fillText('婷婷的拼豆工坊', titleStartX, mainTitleY);
    ...
    ctx.fillText('拼豆图纸生成与作品记录', titleStartX, subTitleY);
    ...
    if (includeStats && colorCounts) {
      // 调整画布大小，包含计算后的统计区域
      const newDownloadHeight = titleBarHeight + extraTopMargin + M * downloadCellSize + (axisLabelSize * 2) + statsHeight + extraBottomMargin + footerBrandHeight;
